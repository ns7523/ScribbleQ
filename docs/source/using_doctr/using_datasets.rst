...
Here are all datasets that are available through hwte:
...
.. code:: python3
    ...
    from hwte.datasets import CORD
    # Load straight boxes
...
.. code:: python3
    ...
    from hwte.datasets import DocArtefacts
...
hwte provides also some generator objects, which can be used to generate synthetic datasets.
Both are also integrated in the training scripts to train a classification or recognition model.

.. code:: python3

    from hwte.datasets import CharacterGenerator
    ds = CharacterGenerator(vocab='abdef', num_samples=100)
    img, target = ds[0]

.. code:: python3

    from hwte.datasets import WordGenerator
...
However, we provide some guidance on how to format your own dataset into the same format so that you can use all hwte tools more easily.
You can find further information about the format in references.

.. code:: python3

    from hwte.datasets import DetectionDataset
...
Each dataset has its specific way to load a sample, but handling batch aggregation and the underlying iterator is a task deferred to another object in hwte.

.. code:: python3

    from hwte.datasets import CORD, DataLoader
    train_set = CORD(train=True, download=True)
