...
from hwte.file_utils import ENV_VARS_TRUE_VALUES
...
def multithread_exec(func: Callable[[Any], Any], seq: Iterable[Any], threads: int | None = None) -> Iterator[Any]:
    ...
    >>> from hwte.utils.multithreading import multithread_exec
    >>> entries = [1, 4, 8]
    ...
    Notes:
        ...
        If you do not have write permissions for this directory (if you run `hwte` on AWS Lambda for instance),
        you might want to disable multiprocessing. To achieve that, set 'hwte_MULTIPROCESSING_DISABLE' to 'TRUE'.
    """
    threads = threads if isinstance(threads, int) else min(16, mp.cpu_count())
    # Single-thread
    if threads < 2 or os.environ.get("hwte_MULTIPROCESSING_DISABLE", "").upper() in ENV_VARS_TRUE_VALUES:
